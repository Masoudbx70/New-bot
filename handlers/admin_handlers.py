...
def setup_admin_handlers(application):
    ...
    application.add_handler(MessageHandler(filters.PHOTO & filters.ChatType.PRIVATE, handle_admin_photo))
...
async def admin_button_handler(update: Update, context):
    ...
    if data == 'admin_verified_list':
        ...
        if pending_users:
            ...
            await query.message.reply_text("✅ هیچ کاربری در انتظار تأیید نیست.")
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.message.reply_text("🖼 مدیریت عکس‌های راهنما", reply_markup=reply_markup)
    
    elif data == 'admin_support':
        # بخش پشتیبانی
        await query.message.reply_text("📞 برای پشتیبانی می‌توانید با آیدی @example_support در ارتباط باشید.")
    
    await query.answer()

async def handle_admin_photo(update: Update, context):
    ...
    await update.message.reply_text("✅ عکس راهنما با موفقیت ذخیره شد.")